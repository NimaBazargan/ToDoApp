...
from .serializers import RegistrationSerializer, CustomAuthTokenSerializer, CustomTokenObtainPairSerializer, ChangePasswordSerializer, ProfileSerializer, ActivationResendSerializer, ResetPasswordSerializer, ResetConfirmSerializer
...
class ResetPasswordApiView(GenericAPIView):
    ...
    def post(self,request):
        ...
        user_obj = serializer.validated_data['user']
        token = self.get_tokens_for_user(user_obj)
        email_obj = EmailMessage('email/reset.tpl', {'token':token}, 'nima.kazemzadeh.bazargan@gmail.com',to=[user_obj.email])
        EmailThread(email_obj).start()
        return Response({'detail':'you can reset your password via the link which sent to your email'},status=status.HTTP_200_OK)
